...
*/
            if (ok && pointInTriangle2(data->verticess[j], data->verticess[data->newfaces[i][0]], data->verticess[data->newfaces[i][1]], data->verticess[data->newfaces[i][2]], 0)){
                ...
                for (k=0; k < data->index_fpv[j]; k++)
                    data->aout[i] += area( data->verticeso[data->faces[data->faces_per_vertex[j][k]][0]], data->verticeso[data->faces[data->faces_per_vertex[j][k]][1]], data->verticeso[data->faces[data->faces_per_vertex[j][k]][2]]);
...
void downsample(int finter[], int nverticesinter, int nfacesinter, float voin[], float vsin[], int fin[], int nvertices, int nfaces, int maxfacespervertex, mxArray ** faces_out, mxArray ** vertices_per_face, mxArray ** area_per_face, mxArray ** mean_per_face, mxArray ** normal_per_face, mxArray ** faces_map){
...
*/
    ...
    double vstep = meanedge/2.0;
    double minvstep = 0.01, mindiff = 0.001;
    double laststd = 0.0, bkplaststd = 0.0, lastdiff = 0.0;
    double prevmaxlen = 0.0;
    double aideal = 3.0 * aoritotal / (double) nnewfaces;

    MEAN = fopen(MEAN_FILE, "w");

    /* Allocate memory to save the intermediate state to be able to roll-back in the case of a bad move */
    float * bkp_aout = (float*) mxGetData(mxCreateNumericMatrix(nnewfaces, 1, mxSINGLE_CLASS, mxREAL)); /* TODO free*/
...
void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    if (nrhs!=4)
        mexErrMsgTxt("Usage: [new_faces, vertices_per_face, area_per_face, means, normals, face_map] = renorm_invert(faces, sphere_vertices, original_vertices , [ previous_faces | initial_number_of_faces ])");
    ...
    downsample(faces_inter_in_p, nverticesinter, nfacesinter, vertices_orig_in_p, vertices_sphere_in_p, faces_in_p, nvertices, nfaces, max, &FACES_OUT, &VERTICES_PER_FACE, &AREA_PER_FACE, &MEAN_PER_FACE, &NORMAL_PER_FACE, &FACE_MAP);
