function [cfg radius]= renorm_simulate_orig(vertices, faces, cfg, varargin)
...
if limsup > liminf
    ...
    active = 2;
    [~, hot radius] = renorm_rand_dip(cfg, limsup, liminf, vertices);
else
...
if go
    ...
    cfg.elec.tri = convhulln(cfg.elec.elecpos);
    f2 = figure;

    patch(struct('vertices', vertices, 'faces', faces, 'FaceColor', 'g', 'FaceAlpha', 0, 'EdgeAlpha', 0.1));
    hold all;
    patch('faces', cfg.elec.tri, 'vertices', cfg.elec.elecpos, 'FaceVertexCData', cfg.rpot(:,init), 'FaceColor', 'interp', 'FaceAlpha', 0.5);
...
%     hold all;
%     patch('faces', cfg.elec.tri, 'vertices', cfg.elec.elecpos, 'FaceVertexCData', cfg.pot(:,init), 'FaceColor', 'interp', 'FaceAlpha', 0.5);
% 
%     for i=1:size(faces,1)
%         if ismember(i, hot)
%             v = cfg.dip.pos(i,:) + 50*cfg.dip.rmom(i)*cfg.dip.mom(i,:);
...
%         hold all;
%         patch('faces', cfg.elec.tri, 'vertices', cfg.elec.elecpos, 'FaceVertexCData', cfg.diff(:,ceil(T/2)), 'FaceColor', 'interp', 'FaceAlpha', 0.5);
%  
%         drawnow;
% 
%         t=0.1;
